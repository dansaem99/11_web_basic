...
SELECT PRICE, PRODUCT_NM, PRODUCT_CD, REG_DT
FROM  SELECT_EX;
		
# 예시 7) 테이블에 존재하지 않는 연산 데이터를 조회 할 수 있다. AS(별명) 키워드로 별칭을 지정할 수 있으며 별칭은 생략할 수 있다.

SELECT PRICE, PRICE * 0.1 AS ADDITIONAL_TAX, PRICE + PRICE * 0.1 AS TOTAL_PRICE
FROM SELECT_EX;
